...
const RuxIconLocalConvenienceStore$1 = /*@__PURE__*/ proxyCustomElement(class extends HTMLElement {
  constructor() {
    super();
    this.__registerHost();
    /**
...
}, [0, "rux-icon-local-convenience-store", {
    "size": [1]
  }]);
function defineCustomElement$1() {
  ...
  const components = ["rux-icon-local-convenience-store"];
  components.forEach(tagName => { switch (tagName) {
    case "rux-icon-local-convenience-store":
      if (!customElements.get(tagName)) {
        customElements.define(tagName, RuxIconLocalConvenienceStore$1);
      }
      break;
  } });
}

const RuxIconLocalConvenienceStore = RuxIconLocalConvenienceStore$1;
const defineCustomElement = defineCustomElement$1;

export { RuxIconLocalConvenienceStore, defineCustomElement };
