...
const index$2 = require('./index-c37ac937.js');
const index = require('./index-117877ea.js');
const index$1 = require('./index-81f27cc2.js');
const index$3 = require('./index-953a2c41.js');
...
function differenceInDays(dirtyDateLeft, dirtyDateRight) {
  ...
  var difference = Math.abs(index$1.differenceInCalendarDays(dateLeft, dateRight));
...
async function validateTimezone(timezone) {
  return new Promise((resolve, reject) => {
    try {
      index$2.formatInTimeZone(new Date(), timezone, 'MM/dd');
      resolve(true);
    }
    catch (e) {
      reject(false);
...
function dateRange(start, end, interval, intervalValue = 1, timezone = 'UTC') {
  ...
  if (interval === 'day') {
    ...
    const output = [...Array(days).keys()].map((i) => {
      ...
      const formattedTime = index$2.formatInTimeZone(time, timezone, 'MM/dd');
      return formattedTime;
    });
    return output;
  }
  if (interval === 'hour') {
    let days = index$3.differenceInHours(endDate, startDate);
    days = days / intervalValue;
    const output = [...Array(days).keys()].map((i) => {
      const time = addHours(startDate, i);
      const formattedTime = index$2.formatInTimeZone(time, timezone, 'HH:mm');
      return formattedTime;
    });
    return output;
  }
