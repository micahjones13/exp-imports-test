...
const RuxDialog = class {
  constructor(hostRef) {
    ...
    this.ruxDialogOpened = index.createEvent(this, "ruxdialogopened", 7);
    this.ruxDialogClosed = index.createEvent(this, "ruxdialogclosed", 7);
    /**
    ...
    this.hasFooter = utils.hasSlot(this.element, 'footer');
    this.hasHeader = utils.hasSlot(this.element, 'header');
    this.hasMessage = utils.hasSlot(this.element);
  ...
  render() {
    ...
    return (open && (index.h(index.Host, null, index.h("div", { part: "container", class: "rux-dialog__wrapper" }, index.h("dialog", { class: "rux-dialog__dialog", role: "dialog", part: "dialog" }, index.h("header", { class: {
        hidden: !this.hasHeader && header === undefined,
        'rux-dialog__header': true,
      }, part: "header" }, index.h("slot", { name: "header", onSlotchange: this._handleSlotChange }, header)), index.h("div", { class: "rux-dialog__content" }, index.h("div", { class: {
        hidden: !this.hasMessage &&
          message === undefined,
        'rux-dialog__message': true,
      }, part: "message" }, index.h("slot", { onSlotchange: this._handleSlotChange }), !this.hasMessage && message ? (index.h("div", null, message)) : null)), index.h("footer", { class: {
        'rux-dialog__footer': true,
      }, part: "footer" }, this.hasFooter ? (index.h("slot", { name: "footer", onSlotchange: this._handleSlotChange })) : (index.h(index.Fragment, null, index.h("rux-button-group", { "h-align": "right" }, index.h("rux-button", { secondary: confirmText.length > 0, onClick: _handleDialogChoice, "data-value": "false", hidden: !denyText, tabindex: "0", exportparts: "container:deny-button", id: "rux-dialog-deny-button" }, denyText), index.h("rux-button", { onClick: _handleDialogChoice, hidden: !confirmText, "data-value": "true", tabindex: "1", exportparts: "container:confirm-button" }, confirmText)), index.h("slot", { name: "footer", onSlotchange: this._handleSlotChange })))))))));
  }
