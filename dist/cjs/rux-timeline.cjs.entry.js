...
const index$2 = require('./index-bc114c83.js');
const helpers = require('./helpers-15f03cb1.js');
const index = require('./index-117877ea.js');
const index$1 = require('./index-81f27cc2.js');
const index$3 = require('./index-817854a0.js');
const index$4 = require('./index-953a2c41.js');
...
function subMilliseconds(dirtyDate, dirtyAmount) {
  ...
  var amount = helpers.toInteger(dirtyAmount);
  return helpers.addMilliseconds(dirtyDate, -amount);
}
...
function startOfUTCWeek(dirtyDate, dirtyOptions) {
  ...
  var defaultWeekStartsOn = localeWeekStartsOn == null ? 0 : helpers.toInteger(localeWeekStartsOn);
  var weekStartsOn = options.weekStartsOn == null ? defaultWeekStartsOn : helpers.toInteger(options.weekStartsOn); // Test if weekStartsOn is between 0 and 6 _and_ is not NaN
...
function getUTCWeekYear(dirtyDate, dirtyOptions) {
  index.requiredArgs(1, arguments);
  var date = index.toDate(dirtyDate, dirtyOptions);
  var year = date.getUTCFullYear();
  var options = dirtyOptions || {};
  var locale = options.locale;
  var localeFirstWeekContainsDate = locale && locale.options && locale.options.firstWeekContainsDate;
  var defaultFirstWeekContainsDate = localeFirstWeekContainsDate == null ? 1 : helpers.toInteger(localeFirstWeekContainsDate);
  var firstWeekContainsDate = options.firstWeekContainsDate == null ? defaultFirstWeekContainsDate : helpers.toInteger(options.firstWeekContainsDate); // Test if weekStartsOn is between 1 and 7 _and_ is not NaN
...
function startOfUTCWeekYear(dirtyDate, dirtyOptions) {
  ...
  var localeFirstWeekContainsDate = locale && locale.options && locale.options.firstWeekContainsDate;
  var defaultFirstWeekContainsDate = localeFirstWeekContainsDate == null ? 1 : helpers.toInteger(localeFirstWeekContainsDate);
  var firstWeekContainsDate = options.firstWeekContainsDate == null ? defaultFirstWeekContainsDate : helpers.toInteger(options.firstWeekContainsDate);
...
const RuxTimeline = class {
  ...
  get totalColumns() {
    ...
    const range = helpers.dateRange(this.start, this.end, this.interval);
    return range.length;
  }
  /**
  ...
  _calculatePlayheadFromTime(time) {
    ...
    let newTime = Math.abs(index$3.differenceInMinutes(new Date(this.start), new Date(time)));
    if (this.interval === 'day') {
      newTime = Math.abs(index$4.differenceInHours(new Date(this.start), new Date(time)));
    }
  ...
  _updateRegions() {
    ...
    if (rulerTrack) {
      ...
      if (rulerEl) {
        helpers.validateTimezone(this.timezone).then(() => {
  ...
  render() {
    return (index$2.h(index$2.Host, null, index$2.h("div", { style: { position: 'relative' } }, index$2.h("div", { class: "rux-timeline", onMouseMove: (ev) => this._handleMouse(ev), onScroll: () => this._handleScroll(), ref: (el) => (this.timelineContainer = el), part: "time-region-container" }, this.playhead && (index$2.h("div", { class: {
        'rux-playhead': true,
        hidden: !this.showPlayhead,
      }, part: "playhead", style: {
        top: `${this.playheadHeight}px`,
        left: `${this.playheadPositionInPixels}px`,
      } })), index$2.h("div", { class: "events", ref: (el) => (this.slotContainer = el) }, index$2.h("slot", { onSlotchange: this._handleSlotChange })), index$2.h("div", { class: "ruler", ref: (el) => (this.rulerContainer = el) }, index$2.h("slot", { name: "ruler" }))))));
  }
...
exports.rux_timeline = RuxTimeline;
