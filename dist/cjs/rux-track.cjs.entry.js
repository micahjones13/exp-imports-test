...
const index$1 = require('./index-817854a0.js');
const index$2 = require('./index-953a2c41.js');
...
const RuxTrack = class {
  ...
  _calculateGridColumnFromTime(time) {
    if (this.start) {
      ...
      if (this.interval === 'hour') {
        const difference = Math.abs(index$1.differenceInMinutes(timelineStart, new Date(time)));
        return difference + 2;
      }
      if (this.interval === 'day') {
        const difference = Math.abs(index$2.differenceInHours(timelineStart, new Date(time)));
        return difference + 2;
      }
    }
    return 0;
  ...
  render() {
    return (index.h(index.Host, null, index.h("div", { class: "rux-timeline rux-track", style: {
        gridTemplateColumns: `[header] 200px repeat(${this.columns}, ${this.width}px)`,
      }, part: "container" }, index.h("div", { class: "rux-track__header", part: "track-header", style: {
        gridRow: '1',
      } }, index.h("slot", { name: "label" })), index.h("slot", { onSlotchange: this._handleSlotChange }))));
  }
